...
final class FLBuilder {
	...
	static public function render_settings_field($name, $field, $settings = null)
	{
		...
		$responsive         = isset($field['responsive']) ? $field['responsive'] : false;
	...
	static public function render_global_css()
	{
		...
		// Global row margins
		if ( '' !== $global_settings->row_margins ) {
			$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->row_margins ) );
			$css .= '.fl-row-content-wrap { margin: ' . esc_attr( $value );
			$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
		}

		// Global row padding
		if ( '' !== $global_settings->row_padding ) {
			$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->row_padding ) );
			$css .= '.fl-row-content-wrap { padding: ' . esc_attr( $value );
			$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
		}

		// Global row width
		if ( '' !== $global_settings->row_width ) {
			$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->row_width ) );
			$css .= '.fl-row-fixed-width { max-width: ' . esc_attr( $value );
			$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
		}

		// Global module margins
		if ( '' !== $global_settings->module_margins ) {
			$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->module_margins ) );
			$css .= '.fl-module-content { margin: ' . esc_attr( $value );
			$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
		}
		...
		if ( $global_settings->responsive_enabled ) {
			...
			$css .= '@media (max-width: '. $global_settings->medium_breakpoint .'px) { ';
				...
				// Global row medium device margins
				if ( isset( $global_settings->row_margins_medium ) && '' !== $global_settings->row_margins_medium ) {
					$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->row_margins_medium ) );
					$css .= '.fl-row-content-wrap { margin: ' . esc_attr( $value );
					$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
				}

				// Global row medium device padding
				if ( isset( $global_settings->row_padding_medium ) && '' !== $global_settings->row_padding_medium ) {
					$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->row_padding_medium ) );
					$css .= '.fl-row-content-wrap { padding: ' . esc_attr( $value );
					$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
				}

				// Global module medium device margins
				if ( isset( $global_settings->module_margins_medium ) && '' !== $global_settings->module_margins_medium ) {
					$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->module_margins_medium ) );
					$css .= '.fl-module-content { margin: ' . esc_attr( $value );
					$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
				}
			...
			$css .= '@media (max-width: '. $global_settings->responsive_breakpoint .'px) { ';
				...
				// Global row responsive device margins
				if ( isset( $global_settings->row_margins_responsive ) && '' !== $global_settings->row_margins_responsive ) {
					$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->row_margins_responsive ) );
					$css .= '.fl-row[data-node] .fl-row-content-wrap { margin: ' . esc_attr( $value );
					$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
				}

				// Global row responsive device padding
				if ( isset( $global_settings->row_padding_responsive ) && '' !== $global_settings->row_padding_responsive ) {
					$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->row_padding_responsive ) );
					$css .= '.fl-row[data-node] .fl-row-content-wrap { padding: ' . esc_attr( $value );
					$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
				}

				// Global module responsive device margins
				if ( isset( $global_settings->module_margins_responsive ) && '' !== $global_settings->module_margins_responsive ) {
					$value = preg_replace( self::regex( 'css_unit' ), '', strtolower( $global_settings->module_margins_responsive ) );
					$css .= '.fl-module-content { margin: ' . esc_attr( $value );
					$css .= ( is_numeric( $value ) ) ? ( 'px; }' ) : ( '; }' );
				}
		...
		foreach ( array( 'default', 'medium', 'responsive' ) as $breakpoint ) {
			...
			if ( ! empty( $breakpoint_css ) ) {
				...
				if ( 'responsive' === $breakpoint ) {
					$selector = $selector_prefix . '.fl-' . $node->type . $selector_suffix;
				}
				else {
